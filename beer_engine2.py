...
class beer_engine_mainwin(object):
	u''' WWW Main Window Creation '''
	def __init__(self, master=None):
		u''' Tab creation + 1st tab creation '''
		...
		if not os.path.isfile(resource_path(u'defaults.txt')):
			with open(resource_path(u'defaults.txt'), u'w') as f:
				...
				evaporation = round(
					(brew_data.constants[u'Boil Volume Scale'] - 1) * 100, 1)
				...
				f.write((u'efficiency={efficiency}\n'
						 u'volume={volume}\n'
						 u'evaporation={evaporation}\n'
						 u'LGratio={LGratio}\n'
						 u'attenuation={attenuation}\n'
						 u'save_close={save_close}\n'
						 u'boil_time={boil_time}\n'
						 u'replace_defaults={replace_defaults}').format(
			...
			with open(resource_path(u'defaults.txt'), u'r') as f:
				...
				for constants in data:
					if constants[0] == u'efficiency':
						brew_data.constants[u'Efficiency'] = float(
							constants[1]) / 100
					elif constants[0] == u'volume':
						brew_data.constants[u'Volume'] = float(constants[1])
					elif constants[0] == u'evaporation':
						brew_data.constants[u'Boil Volume Scale'] = (
							float(constants[1]) / 100) + 1
					elif constants[0] == u'LGratio':
						brew_data.constants[u'Liquor To Grist Ratio'] = float(
							constants[1])
					elif constants[0] == u'attenuation':
						brew_data.constants[u'Attenuation Default'] = constants[1]
					elif constants[0] == u'save_close':
						brew_data.constants[u'Save On Close'] = True if constants[1] == u'True' else False
					elif constants[0] == u'boil_time':
						brew_data.constants[u'Default Boil Time'] = int(
							constants[1])
					elif constants[0] == u'replace_defaults':
						brew_data.constants[u'Replace Defaults'] = True if constants[1] == u'True' else False
		...
		if not os.path.isfile(resource_path(u'hop_data.txt')):
			with open(resource_path(u'hop_data.txt'), u'w') as f:
				for hop, value in brew_data.hop_data.items():
					...
					f.write(
						u'{name}\t{hop_type}\t{origin}\t{alpha}\t{use}\t{description}\n'.format(
							name=name,
							hop_type=hop_type,
							origin=origin,
							alpha=alpha,
			...
			with open(resource_path(u'hop_data.txt'), u'r') as f:
				if brew_data.constants[u'Replace Defaults']:
					brew_data.hop_data = {}
				data = [line.strip().split(u'\t') for line in f]
				for hop in data:
		...
		if not os.path.isfile(resource_path(u'grain_data.txt')):
			with open(resource_path(u'grain_data.txt'), u'w') as f:
				for ingredient, value in brew_data.grist_data.items():
					...
					description = value[u'Description']
					f.write(
						u'{name}\t{ebc}\t{grain_type}\t{extract}\t{moisture}\t{fermentability}\t{description}\n'.format(
							name=name,
							ebc=ebc,
		...
		if not os.path.isfile(resource_path(u'yeast_data.txt')):
			with open(resource_path(u'yeast_data.txt'), u'w') as f:
				...
				for yeast, value in brew_data.yeast_data.items():
					...
					description = value[u'Description']
					f.write(
						u'{name}\t{yeast_type}\t{lab}\t{flocculation}\t{attenuation}\t{temperature}\t{origin}\t{description}\n'.format(
							name=name,
		...
		if not os.path.isfile(resource_path(u'water_chem_data.txt')):
			...
			with open(resource_path(u'water_chem_data.txt'), u'r') as f:
				...
				for water_chem in data:
					name = water_chem[0]
					time = float(
						water_chem[1]) if water_chem[1] != u'N/A' else water_chem[1]
					water_chem_type = water_chem[2]
					brew_data.water_chemistry_additions[name] = {
						u'Values': {u'Type': water_chem_type}}
					if time != u'N/A':
						brew_data.water_chemistry_additions[name][u'Values'][u'Time'] = time
		...
		if not platform.system() == u'Darwin':
			self.master.tk.call(
				u'wm', u'iconphoto', self.master._w, tk.PhotoImage(
					file=resource_path(u'logo.png')))
		...
		self.file_menu.add_command(
			...
			command=lambda: self.save_file(
				filedialog.asksaveasfilename(
					...
					title=u"Select file",
					defaultextension=u".berfx",
					initialfile=u'{0}.berf'.format(
						self.recipe_name_ent.get()))),
		...
		self.master.bind(
			u"<Control-h>",
			lambda e: webbrowser.open_new(u'https://github.com/jimbob88/wheelers-wort-works/wiki'))
		...
		self.boil_vol.set(
			unicode(brew_data.constants[u'Volume'] * brew_data.constants[u'Boil Volume Scale']))
		...
		# self.calc_lbl.configure(anchor='nw')
		default_text = u'''Efficiency: {efficiency}%{enter}Final Gravity: {final_gravity}{enter}Alcohol (ABV): {abv}{enter}Colour: {colour}EBC{enter}Mash Liquor: {mash_liquor}L{enter}IBU:GU: {ibu_gu}'''.format(
			efficiency=brew_data.constants[u'Efficiency'] * 100, final_gravity=1.000, abv=0, colour=0, mash_liquor=0, ibu_gu=0, enter=u'\n\n')
		...
		self.ingredients_imperial_chk_butt.place(
			relx=0.442, rely=0.402, relheight=0.044, relwidth=0.149)
		...
		self.ingredients_imperial_chk_butt.configure(
			command=self.ingredient_to_imperial)
		self.ingredients_imperial_chk_butt.configure(
			variable=self.is_imperial_ingredient)
	...
	def refresh_tab_onclick(self, event):
		u''' When tab is clicked, refresh/reinsert '''
		tab = self.tabbed_frame.tk.call(
			self.tabbed_frame._w, u"identify", u"tab", event.x, event.y)
		if tab == 0:
			self.refresh_all()
		elif tab == 1:
			self.second_tab.reinsert()
	...
	def refresh_grist(self):
								...
								width=max(
									[
										len(
											u'{lb}:{oz}'.format(
												lb=int(
													ingredient[u'Values'][u'lb:oz'][0]),
												oz=round(
													ingredient[u'Values'][u'lb:oz'][1],
		...
		def refresh_percentage():
			u''' Refresh Percentage based off weight '''
			total_weight = sum([ingredient[u'Values'][u'Grams']
		...
		def refresh_orig_grav():
			u''' Refresh Original Gravity '''
			non_mashables = [6.0, 5.0]
			volume = float(self.volume.get())
			points = sum([(brew_data.grist_data[ingredient[u'Name']][u'Extract'] * (ingredient[u'Values'][u'Grams']) / 1000) * (
				1 if brew_data.grist_data[ingredient[u'Name']][u'Type'] in non_mashables else brew_data.constants[u'Efficiency']) for ingredient in self.ingredients])
		...
		def refresh_indiv_grav():
			u''' Refresh Individual Gravity of Ingredients '''
			non_mashables = [6.0, 5.0]
			volume = float(self.volume.get())
			for ingredient in self.ingredients:
				points = brew_data.grist_data[ingredient[u'Name']
											  ][u'Extract'] * (ingredient[u'Values'][u'Grams']) / 1000
				eff = (1 if brew_data.grist_data[ingredient[u'Name']][u'Type']
					   in non_mashables else brew_data.constants[u'Efficiency'])
		...
		if not self.is_ogfixed.get():
			...
			non_mashables = [6.0, 5.0]
			factor = sum([ingredient[u'Values'][u'Percent'] * brew_data.grist_data[ingredient[u'Name']][u'Extract'] * (1 if brew_data.grist_data[ingredient[u'Name']]
																												   [u'Type'] in non_mashables else brew_data.constants[u'Efficiency']) for idx, ingredient in enumerate(self.ingredients)])
			...
			for idx, ingredient in enumerate(self.ingredients):
				...
				self.ingredients[idx] = {
					u'Name': ingredient[u'Name'], u'Values': {
						u'EBC': EBC, u'Grav': 0.0, u'lb:oz': (
							lb, oz), u'Grams': weight, u'Percent': percent}}
			refresh_percentage()

		refresh_indiv_grav()
		for ingredient in self.ingredients:
			values = (
				ingredient[u'Values'][u'EBC'], round(
					ingredient[u'Values'][u'Grav'], 1), u'{lb}:{oz}'.format(
					lb=int(
						ingredient[u'Values'][u'lb:oz'][0]), oz=round(
						ingredient[u'Values'][u'lb:oz'][1], 1)), round(
						ingredient[u'Values'][u'Grams'], 1), ingredient[u'Values'][u'Percent'])
			self.scrolled_tree_ingredient.insert(
				u'', u'end', text=ingredient[u'Name'], values=values)
	...
	def refresh_hop(self):
		...
		def make_treeview():
			...
			if u'scrolled_tree_hops' in vars(self):
				...
				self.scrolled_tree_hops = ScrolledTreeView(
					self.frame_hops, style=u"mystyle.Treeview")
				...
				self.scrolled_tree_hops.heading(
					u"#0", text=u"Hop Variety", command=lambda: self.sort_by_hop(u"Hop Variety"))
				self.scrolled_tree_hops.column(
					u"#0", width=u"90", anchor=u"w", minwidth=u"20", stretch=u"1")

				for column in self.hop_columns:
					self.scrolled_tree_hops.heading(
						column, text=column, command=lambda column=column: self.sort_by_hop(column))
					if column != u'lb:oz' and column != u'%':
						self.scrolled_tree_hops.column(
							column, width=40, anchor=u"center")
					elif column == u'lb:oz':
						if len(self.hops) > 0:
							self.scrolled_tree_hops.column(
								column, width=max(
									[
										len(
											u'{lb}:{oz}'.format(
												lb=int(
													hop[u'Values'][u'lb:oz'][0]), oz=round(
													hop[u'Values'][u'lb:oz'][1], 1))) for hop in self.hops]) * 7, anchor=u"center")
						else:
							self.scrolled_tree_hops.column(
								column, width=40, anchor=u"center")
					elif column == u'%' or column == u'% Util':
						self.scrolled_tree_hops.column(
							column, width=35, anchor=u"center")
		...
		def refresh_util():
			u''' Refresh Individual Utilization of Hops '''
			def boil_grav():
				u''' Get Boil Gravity of Recipe'''
				volume = float(self.boil_vol.get())
				points = sum([brew_data.grist_data[ingredient[u'Name']][u'Extract'] * (
					ingredient[u'Values'][u'Grams']) / 1000 for ingredient in self.ingredients])
				boil_grav = (
					(points * brew_data.constants[u'Efficiency']) / volume) + 1000
				return boil_grav
			u'''
		...
		def refresh_ibu():
			...
			IBU    =        grams x alpha acid x utilisation rate
					   -------------------------------------------------
									 Volume x 10
			'''
			ibu = sum([(hop[u'Values'][u'Grams'] *
						hop[u'Values'][u'Alpha'] *
						hop[u'Values'][u'Util']) /
					   (float(self.boil_vol.get()) *
						10) for hop in self.hops])
		...
		def refresh_indiv_ibu():
			u''' Refresh Individual IBUs of Hops '''
			for hop in self.hops:
				ibu = (hop[u'Values'][u'Grams'] * hop[u'Values'][u'Alpha'] * \
					   hop[u'Values'][u'Util']) / (float(self.boil_vol.get()) * 10)
		...
		if not self.is_ebufixed.get():
			...
			factor = sum([hop[u'Values'][u'Percent'] * hop[u'Values'][u'Alpha']
						  * hop[u'Values'][u'Util'] for idx, hop in enumerate(self.hops)])
		...
		for hop in self.hops:
			values = (
				hop[u'Values'][u'Type'], hop[u'Values'][u'Alpha'], hop[u'Values'][u'Time'], round(
					hop[u'Values'][u'Util'], 1), round(
					hop[u'Values'][u'ibu']), u'{lb}:{oz}'.format(
					lb=int(
						hop[u'Values'][u'lb:oz'][0]), oz=round(
						hop[u'Values'][u'lb:oz'][1], 1)), round(
							hop[u'Values'][u'Grams'], 1), hop[u'Values'][u'Percent'])
			self.scrolled_tree_hops.insert(
				u'', u'end', text=hop[u'Name'], values=values)

	def refresh_all(self):
		u''' Run all refresh and recalculate commands '''
		self.refresh_hop()
		self.refresh_grist()
		self.recalculate()

	def add_grist(self):
		u''' Add a grist to the Fermentable Ingredients Table '''
		def insert():
			u''' Append Grist to self.ingredients and then refresh '''
			name = grist_options.item(grist_options.focus())[u'text']
			EBC = int(brew_data.grist_data[name][u'EBC'])
			self.ingredients.append({u'Name': name, u'Values': {
									u'EBC': EBC, u'Grav': 0.0, u'lb:oz': (0.0, 0.0), u'Grams': 0, u'Percent': 0.0}})
			self.refresh_grist()

		# https://mail.python.org/pipermail/python-list/2002-May/170135.html
		# ADDED TREEVIEW functionality
		def bound(event, treeview, list_data):
			u''' Reprogrammed code that accepts letter (jump to letter) '''
			key = event.keysym
			if key == u'Escape':
					...
					for n in xrange(start_n + 1, len(list_data)):
						...
						if item[0].lower() == key.lower():
							treeview.selection_set(
								u'I{iid}'.format(iid=format(n + 1, u'03x')))
							treeview.focus(u'I{iid}'.format(
								iid=format(n + 1, u'03x')))
							treeview.yview(n)
							return
						treeview.yview(n)
					else:
		...
		grist_options = ScrolledTreeView(
			add_grist_gui, show=u"tree", columns=(u"EBC"))
	...
	def add_hop(self):
		u''' Add a hop to the "Hop Variety" Table '''
		def insert():
			u''' Append hop to self.hops and then refresh '''
			...
			self.hops.append({u'Name': name,
							  u'Values': {u'Type': type,
										 u'Alpha': alpha,
										 u'Time': time,
										 u'Util': 0.0,
										 u'ibu': 0.0,
		...
		hop_options = ScrolledTreeView(
			add_hop_gui, show=u"tree", columns=(u"Form"))
		hop_options.column(column=u"Form", width=80)
		hop_options.grid(row=1, column=0)
		for hop in sorted(brew_data.hop_data):
			hop_options.insert(
				u'', tk.END, text=(hop), values=(
					brew_data.hop_data[hop][u'Form'],))
	...
	def add_weight_ingredients(self, weight):  # Selected Item
		u''' Add/Remove Mass to/from Ingredients '''
		try:
			selection = self.scrolled_tree_ingredient.selection()[0]
			ingred_id = int(unicode(selection)[1:], 16)
			#print(id, selection)
			grams = self.ingredients[ingred_id - 1][u'Values'][u'Grams'] + weight
			if grams < 0:
				grams = 0
			lb = grams / brew_data.constants[u'Conversion'][u'lb-g']
			oz = (lb - int(lb)) * 16
			self.ingredients[ingred_id - 1][u'Values'][u'Grams'] = grams
			self.ingredients[ingred_id - 1][u'Values'][u'lb:oz'] = (lb, oz)
			self.refresh_grist()
			self.recalculate()
	...
	def add_weight_hops(self, weight):  # Selected Item
		u''' Add/Remove Mass to/from Hops '''
		try:
			selection = self.scrolled_tree_hops.selection()[0]
			hop_id = int(unicode(selection)[1:], 16)
			grams = self.hops[id - 1][u'Values'][u'Grams'] + weight
			if grams < 0:
				grams = 0
			lb = grams / brew_data.constants[u'Conversion'][u'lb-g']
			oz = (lb - int(lb)) * 16
			self.hops[hop_id - 1][u'Values'][u'Grams'] = grams
			self.hops[hop_id - 1][u'Values'][u'lb:oz'] = (lb, oz)
			self.refresh_hop()
			self.recalculate()
	...
	def zero_ingredients(self, curr_selection=None):
		u''' Zero Data Values in Ingredient '''
		try:
			...
			ingred_id = int(unicode(selection)[1:], 16)
			EBC = int(
				brew_data.grist_data[self.ingredients[ingred_id - 1][u'Name']][u'EBC'])
			self.ingredients[ingred_id - 1] = {u'Name': self.ingredients[ingred_id - 1][u'Name'], u'Values': {
				u'EBC': EBC, u'Grav': 0.0, u'lb:oz': (0.0, 0.0), u'Grams': 0, u'Percent': 0.0}}
			self.refresh_grist()
			self.recalculate()
			self.scrolled_tree_ingredient.focus_set()
	...
	def zero_hops(self):
		u''' Zero Data Values in Hop '''
		try:
			selection = self.scrolled_tree_hops.selection()[0]
			hop_id = int(unicode(selection)[1:], 16)
			alpha = brew_data.hop_data[self.hops[hop_id - 1][u'Name']][u'Alpha']
			type = brew_data.hop_data[self.hops[hop_id - 1][u'Name']][u'Form']
			self.hops[hop_id - 1] = {u'Name': self.hops[hop_id - 1][u'Name'],
								 u'Values': {u'Type': type,
											u'Alpha': alpha,
											u'Time': 0.0,
											u'Util': 0.0,
	...
	def add_time(self, time):
		u''' Add/Remove Time to/from Hops '''
		try:
			selection = self.scrolled_tree_hops.selection()[0]
			hop_id = int(unicode(selection)[1:], 16)
			...
			if time < 0:
				time = 0
			self.hops[hop_id - 1][u'Values'][u'Time'] = time
			self.refresh_hop()
			self.recalculate()
			self.scrolled_tree_hops.focus_set()
			self.scrolled_tree_hops.see(selection)
	...
	def add_alpha(self, alpha):
		u''' Add/Remove Alpha to/from Hops '''
		try:
			selection = self.scrolled_tree_hops.selection()[0]
			id = int(unicode(selection)[1:], 16)
			alpha = round(self.hops[hop_id - 1][u'Values'][u'Alpha'] + alpha, 1)
			if alpha < 0:
				alpha = 0
			self.hops[hop_id - 1][u'Values'][u'Alpha'] = alpha
			self.refresh_hop()
			self.recalculate()
			self.scrolled_tree_hops.focus_set()
			self.scrolled_tree_hops.see(selection)
	...
	def recalculate(self):
		u''' Recalculate main items in the Engine Room '''
		def final_gravity():
			u''' Calculate the Final Gravity of Recipe '''
			non_mashables = [6.0, 5.0]
			a = sum([((self.attenuation_apply(ingredient) / 100) * ((ingredient[u'Values'][u'Grams'] / 1000) * brew_data.grist_data[ingredient[u'Name']][u'Extract']))
					 * (1 if brew_data.grist_data[ingredient[u'Name']][u'Type'] in non_mashables else brew_data.constants[u'Efficiency']) for ingredient in self.ingredients])
			b = sum([(((100 - self.attenuation_apply(ingredient)) / 100) * ((ingredient[u'Values'][u'Grams'] / 1000) * brew_data.grist_data[ingredient[u'Name']][u'Extract']))
					 * (1 if brew_data.grist_data[ingredient[u'Name']][u'Type'] in non_mashables else brew_data.constants[u'Efficiency']) for ingredient in self.ingredients])
			return ((b - (a * 0.225)) / float(self.volume.get())) + 1000

		def alcohol_by_volume(og, fg):
			u''' Calculate the Alcohol Content of Recipe '''
			# return (1.05/0.79) * ((og - fg) / fg) *100
			return (((1.05 * (og - fg)) / fg / 0.79)) * 100

		def mash_liquor():
			u''' Calculate the Mash Liquor of Recipe '''
			non_mashables = [6.0, 5.0]  # ["Copper Sugar", "Malt Extract"]
			grist_mass = sum([0 if brew_data.grist_data[ingredient[u'Name']][u'Type']
							  in non_mashables else ingredient[u'Values'][u'Grams'] for ingredient in self.ingredients]) / 1000
			return grist_mass * brew_data.constants[u'Liquor To Grist Ratio']

		def colour_ebc():
			u''' Calculate the EBC (COLOUR) of Recipe '''
			# [{'Name:': 'Wheat Flour', 'Values': {'EBC:': 0.0, 'Grav': 0.0, 'lb:oz': (0.0,0.0), 'Grams': 0.0, 'Percent': 0.0}}]
			def formula(ingredient, efficiency):
				u''' Defines the formula for calulation (cleaner) '''
			...
			non_mashables = [6.0, 5.0]
			return (sum([formula(ingredient, 1) if brew_data.grist_data[ingredient[u'Name']][u'Type'] in non_mashables else formula(
				ingredient, brew_data.constants[u'Efficiency']) for ingredient in self.ingredients]))
		...
		self.calc_lbl.insert(
			u'end',
			u'''Efficiency: {efficiency}%{enter}Final Gravity: {final_gravity}{enter}Alcohol (ABV): {abv}{enter}Colour: {colour}EBC{enter}Mash Liquor: {mash_liquor}L{enter}IBU:GU: {ibu_gu}'''.format(
				efficiency=round(
					brew_data.constants[u'Efficiency'] *
					100,
					13),
				final_gravity=round(
					self.fg,
	...
	def ingredient_to_imperial(self):
		u''' Convert Ingredient Buttons to/from Imperial Units '''
		if self.is_imperial_ingredient.get() == 0:
			...
			self.add_1g_ing_butt.configure(
				command=lambda: self.add_weight_ingredients(
					brew_data.constants[u'Conversion'][u'oz-g'] / 16))
	...
	def hop_to_imperial(self):
		u''' Convert Hop Buttons to/from Imperial Units '''
		if self.is_imperial_hop.get() == 0:
			...
			self.add_100g_hop_butt.configure(
				command=lambda: self.add_weight_hops(
					brew_data.constants[u'Conversion'][u'oz-g'] * 4))
			self.add_100g_hop_butt.configure(font=(None, 7))

			self.rem_100g_hop_butt.configure(text=u'''-4oz''')
			self.rem_100g_hop_butt.configure(
				command=lambda: self.add_weight_hops(-brew_data.constants[u'Conversion'][u'oz-g'] * 4))
			...
			self.rem_25g_hop_butt.configure(
				command=lambda: self.add_weight_hops(-brew_data.constants[u'Conversion'][u'oz-g']))
			...
			self.rem_1g_hop_butt.configure(
				command=lambda: self.add_weight_hops(-brew_data.constants[u'Conversion'][u'oz-g'] / 16))
			self.rem_1g_hop_butt.configure(font=(None, 7))

	def attenuation_apply(self, ingredient):
		u''' Apply Attenuation to an Ingredient '''
		# print(brew_data.grist_data[ingredient['Name']]['Fermentability'])
		if int(brew_data.grist_data[ingredient[u'Name']]
			   [u'Fermentability']) == 200:
			table_dict = {
			...
			# print('else')
			atten = brew_data.grist_data[ingredient[u'Name']][u'Fermentability']
		return atten

	def sort_by_grist(self, column):
		u''' Sort Each Column reverse/inverse '''
		# [{'Name:': 'Wheat Flour', 'Values': {'EBC:': 0.0, 'Grav': 0.0, 'lb:oz': (0.0,0.0), 'Grams': 0.0, 'Percent': 0.0}}]
		################################### Grist #############################
		...
		if column == u'Fermentable Ingredient':
			self.ingredients = sorted(
				self.ingredients, key=lambda k: k[u'Name'])
		elif column == u'Ebc':
			self.ingredients = sorted(
				self.ingredients, key=lambda k: k[u'Values'][u'EBC'])
		elif column == u'Grav':
			self.ingredients = sorted(
				self.ingredients,
				key=lambda k: k[u'Values'][u'Grav'])
		elif column == u'lb:oz':
			self.ingredients = sorted(self.ingredients, key=lambda k: (
				k[u'Values'][u'lb:oz'][0] + (k[u'Values'][u'lb:oz'][1] / 16)))
		elif column == u'Grams':
			self.ingredients = sorted(
				self.ingredients,
				key=lambda k: k[u'Values'][u'Grams'])
		elif column == u'%':
			self.ingredients = sorted(
				self.ingredients,
				key=lambda k: k[u'Values'][u'Percent'])
	...
	def sort_by_hop(self, column):
		...
		if column == u'Hop Variety':
			...
			self.hops = sorted(self.hops, key=lambda k: (
				k[u'Values'][u'lb:oz'][0] + (k[u'Values'][u'lb:oz'][1] / 16)))
		elif column == u'Grams':
	...
	def create_html(self, start=u'', open_browser=True, use_sorttable=False):
		u''' Export an HTML File from Recipe '''
		self.recalculate()
		if use_sorttable:
			start += u'<script src="https://www.kryogenix.org/code/browser/sorttable/sorttable.js"></script>'
		start += u'<html><head><title>{name}</title><link rel="shortcut icon" href="{logo}" />'.format(
			name=self.recipe_name_ent.get().replace(u'&', u'&amp;'), logo=resource_path(u'logo.png'))
		start += ur'''
		...
		for addition in self.sixth_tab.added_additions:
			try:
				if brew_data.water_chemistry_additions[addition][u'Values'][u'Type'] == u'Malt':
					start += u'<tr>'
					start += u'<td class="ing1">{name}</td>'.format(
						name=addition)
		...
		for ingredient in self.ingredients:
			start += u'<tr>'
			start += u'<td class="ing1">{name}</td>'.format(
				name=ingredient[u'Name'])
			start += u'<td class="ing2">{colour}</td>'.format(
				colour=ingredient[u'Values'][u'EBC'])
			start += u'<td class="ing3">{lb}:{oz}</td>'.format(
				lb=int(
					ingredient[u'Values'][u'lb:oz'][0]), oz=round(
					ingredient[u'Values'][u'lb:oz'][1], 1))
			start += u'<td class="ing3">{grams}</td>'.format(
				grams=(
					round(
						ingredient[u'Values'][u'Grams'], 1)) if (
					ingredient[u'Values'][u'Grams'] - int(
						ingredient[u'Values'][u'Grams'])) >= 2 else round(
					ingredient[u'Values'][u'Grams']))
			start += u'<td class="ing4">{percentage}%</td>'.format(
				percentage=ingredient[u'Values'][u'Percent'])
			start += u'</tr>'
		start += u'</table><br>'
		if start[-179:] == u'<tr><th class="subhead">Fermentable</th><th class="subhead">Colour</th><th class="subhead">lb:oz</th><th class="subhead">Grams</th><th class="subhead2">Ratio</th></tr></table><br>':
			start = start[:-179]

		if self.sixth_tab.water_boil_is_disabled.get() == 1:
			start += u'<p><b>Boil Time: </b>{boil_time}</p>'.format(
		...
		for addition in self.sixth_tab.added_additions:
			try:
				if brew_data.water_chemistry_additions[addition][u'Values'][u'Type'] == u'Hop':
					temp_hop.append(
						{u'Name': addition, u'Values': brew_data.water_chemistry_additions[addition][u'Values']})
				else:
					temp_hop.append(None)
			except KeyError:
				pass

		temp_hop = list(sorted(
			[x for x in temp_hop if x is not None], key=lambda k: k[u'Values'][u'Time']))
		for hop in reversed(temp_hop):
			if hop[u'Values'][u'Type'] != u'Hop':
				start += u'<tr>'
				start += u'<td class="hop1">{name}</td>'.format(
					name=hop[u'Name'])
				start += u'<td class="hop2">{type}</td>'.format(
					type=hop[u'Values'][u'Type'])
				start += u'<td class="hop3">{alpha}</td>'.format(
					alpha=hop[u'Values'][u'Alpha'])
				start += u'<td class="hop4">{time}</td>'.format(
					time=round(hop[u'Values'][u'Time']))
				start += u'<td class="hop5">{lb}:{oz}</td>'.format(
					lb=int(
						hop[u'Values'][u'lb:oz'][0]), oz=round(
						hop[u'Values'][u'lb:oz'][1], 1))
				start += u'<td class="hop5">{grams}</td>'.format(grams=(round(hop[u'Values'][u'Grams'], 1)) if (
					hop[u'Values'][u'Grams'] - int(hop[u'Values'][u'Grams'])) >= 2 else round(hop[u'Values'][u'Grams']))
				start += u'<td class="hop6">{percentage}%</td>'.format(
				...
				start += u'</tr>'
		start += u'</table><br>'
		if start[-236:] == u'<tr><th class="subhead">Hop Variety</th><th class="subhead">Type</th><th class="subhead">Alpha</th><th class="subhead">Time</th><th class="subhead">lb:oz</th><th class="subhead">Grams</th><th class="subhead2">Ratio</th></tr></table><br>':
			start = start[:-236]
		...
		for addition in self.sixth_tab.added_additions:
			try:
				...
				if len(
					brew_data.yeast_data[addition][u'Temperature'].replace(
						u'°',
						u'').split(u'-')) >= 2:
					temperature = brew_data.yeast_data[addition][u'Temperature'].replace(
						u'°', u'').split(u'-')[0]
					temperature += u'-' + \
						brew_data.yeast_data[addition][u'Temperature'].replace(u'°', u'').split(u'-')[1]
				else:
					temperature = temperature
				...
				start += u'<td class="yst4">{yeast_type}</td>'.format(
					yeast_type=yeast_type)
				start += u'<td class="yst5">{flocculation}</td>'.format(
					flocculation=flocculation)
				start += u'<td class="yst6">{attenuation}</td>'.format(
					attenuation=attenuation)
				start += u'<td class="yst7">{temperature}</td>'.format(
					temperature=temperature)
				start += u'</tr>'
			except KeyError:
				try:
					if brew_data.water_chemistry_additions[addition][u'Values'][u'Type'] == u'Yeast':
						start += u'<tr>'
						start += u'<td class="yst1">{name}</td>'.format(
							name=addition)
					...
					pass
		start += u'</table>'
		if start[-245:] == u'<tr><th class="subhead">Yeast</th><th class="subhead">Lab</th><th class="subhead">Origin</th><th class="subhead">Type</th><th class="subhead">Flocculation</th><th class="subhead">Attenuation</th><th class="subhead2">Temperature</th></tr></table>':
			start = start[:-245]

		start += u'<p><b>Final Volume: </b>{volume} Litres</p>'.format(
			volume=self.volume.get())
		start += u'<p><b>Original Gravity: </b>{og}</p>'.format(
			og=round(self.og, 1))
		start += u'<p><b>Final Gravity: </b>{fg}</p>'.format(
			fg=round(self.fg, 1))
		start += u'<p><b>Alcohol Content: </b>{abv}% ABV</p>'.format(
			abv=round(self.abv, 1))
		start += u'<p><b>Mash Efficiency: </b>{efficiency}</p>'.format(
			efficiency=brew_data.constants[u'Efficiency'] * 100)
		start += u'<p><b>Bitterness: </b>{bitterness} IBU</p>'.format(
			bitterness=round(self.ibu))
		start += u'<p><b>Colour: </b>{colour} EBC</p>'.format(
		...
		if self.seventh_tab.html_formatting.get():
			start += u'''<hr><h2>Notes</h2>\n{notes}'''.format(
				notes=notes) if len(notes) >= 1 else u''
		else:
			start += u'''<hr><h2>Notes</h2>\n<p>{notes}</p>'''.format(
		...
		start = bs4.BeautifulSoup(
			start, features=u"html.parser").prettify() if u'bs4' in sys.modules else start
		text_file_name = resource_path(u'{recipe_name}.html'.format(
			recipe_name=self.recipe_name_ent.get().replace(u'/', u'⁄')))
		with open(text_file_name, u'w') as hs:
			hs.write(start)
		if open_browser:
			webbrowser.open(
				u'file://' +
				os.path.realpath(text_file_name),
				new=1)
	...
	def open_dialog(self):
		...
		file_str = filedialog.askopenfilename(
					...
					filetypes=(
						...
						(u"all files",
						 u"*.*")))
		if file_str == u'' or file_str is None or isinstance(file_str, tuple):
			return
		if self.is_clear():
			self.open_file(file_str)
			return
	...
	def open_file(self, file):
		...
		if file != u'' and file is not None and not isinstance(file, tuple):
			self.sixth_tab.original_additions = list(sorted(
				brew_data.water_chemistry_additions)) + list(sorted(brew_data.yeast_data))
			self.sixth_tab.added_additions = []
			self.sixth_tab.refresh_all()
			examples = [
				u'1920s Bitter',
				u'Bog-Standard Bitter',
				u'Black-Country Mild',
				u'Irish Stout',
				u'1920s Mild',
				u'1920s Porter',
				u'1920s Stock Ale',
			...
			if file.lower().endswith(u'.berf') or file.split(u'/')[-1] in examples:
				...
				with open(file, u'rb') as f:
					...
					data = [line.replace('\xa7', '\t').strip().decode(
						u'ISO-8859-1').split(u'\t') for line in f]
					# print(data)
					for sublist in data:
						if sublist[0] == u'grain':
							...
							EBC = float(sublist[2])
							self.ingredients.append({u'Name': sublist[1], u'Values': {
													u'EBC': EBC, u'Grav': 0, u'lb:oz': (lb, oz), u'Grams': grams, u'Percent': percent}})
							...
							if not any(e[1] == u'boilvol' for e in data):
								...
								self.boil_volume_ent.insert(
									0, float(sublist[2]) * brew_data.constants[u'Boil Volume Scale'])
				...
				with open(file, u'r') as f:
					...
					data = [
						line.replace(
							u'\xa7',
							u'\t').strip().split(u'\t') for line in f]
					for sublist in data:
						if sublist[0] == u'grain':
							self.ingredients.append(
								{u'Name': sublist[1], u'Values': ast.literal_eval(sublist[2])})
						elif sublist[0] == u'hop':
							self.hops.append(
								{u'Name': sublist[1], u'Values': ast.literal_eval(sublist[2])})
						elif sublist[0] == u'add':
			...
			self.sixth_tab.original_additions = sorted(set(self.sixth_tab.original_additions) - set(
				self.sixth_tab.added_additions), key=self.sixth_tab.original_additions.index)
	...
	def save_file(self, file):
		u''' Save Current Recipe as `.berf`/`.berfx` '''
		if file != u'' and isinstance(file, unicode):
			if file.lower()[-5:] == u'.berf':
				...
				with codecs.open(file, u'w', u'ISO-8859-1', errors=u'ignore') as f:
					for ingredient in self.ingredients:
						...
						units = (
							ingredient[u'Values'][u'Grams'] / 1000) * brew_data.constants[u'Efficiency']
						moisture = brew_data.grist_data[ingredient[u'Name']][u'Moisture']
						fermentability = brew_data.grist_data[ingredient[u'Name']
															  ][u'Fermentability']
						grams = ingredient[u'Values'][u'Grams']
						percentage = ingredient[u'Values'][u'Percent']
						f.write(
							u'grain\xa7{name}\t{ebc}\t{type}\t{units}\t{moisture}\t{fermentability}\t{grams}\t{percentage}\n'.format(
								name=ingredient[u'Name'],
								ebc=ebc,
								type=ingred_type,
								units=units,
					...
					for hop in self.hops:
						...
						percentage = hop[u'Values'][u'Percent']
						f.write(
							u'hop\xa7{name}\t{type}\t{alpha}\t{ibu}\t{grams}\t{time}\t{percentage}\n'.format(
								name=hop[u'Name'],
								type=hop_type,
								alpha=alpha,
								ibu=ibu,
					...
					for addition in self.sixth_tab.added_additions:
						...
						f.write(
							u'add\xa7{name}\t{type}\n'.format(
								name=name, type=addition_type))
					f.write(u'default\xa7efficiency\t{efficiency}\n'.format(
						efficiency=brew_data.constants[u'Efficiency'] * 100))
					f.write(
						u'default\xa7volume\t{volume}\n'.format(
							volume=self.volume.get()))
					f.write(
						u'default\xa7boilvol\t{boilvol}\n'.format(
							boilvol=self.boil_vol.get()))
					f.write(
						u'miscel\xa7recipename\t{recipename}\n'.format(
							recipename=self.recipe_name_ent.get()))
					f.write(
						u'miscel\xa7ogfixed\t{ogfixed}\n'.format(
							ogfixed=self.is_ogfixed.get()))
					f.write(
						u'miscel\xa7ebufixed\t{ebufixed}\n'.format(
							ebufixed=self.is_ebufixed.get()))
					f.write(
						u'miscel\xa7origgrav\t{origgrav}\n'.format(
							origgrav=self.og))

					notes = repr(
						self.seventh_tab.texpert.get(
							u'1.0', u'end'))  # , encoding='utf8')
					# print(notes)
				...
				with open(file, u'w') as f:
					for ingredient in self.ingredients:
						f.write(
							u'grain\xa7{name}\t{data}\n'.format(
								name=ingredient[u'Name'],
								data=ingredient[u'Values']))
					for hop in self.hops:
						# 'Values': {'Type': 'Whole', 'Alpha': 12.7, 'Time': 0.0, 'Util': 0.0, 'ibu': 0.0, 'lb:oz': (0.0,0.0), 'Grams': 0.0, 'Percent': 0.0}
						f.write(
							u'hop\xa7{name}\t{data}\n'.format(
					...
					for key, grist in brew_data.grist_data.items():
						f.write(
							u'database\xa7grist\xa7{name}\t{data}\n'.format(
								name=key, data=grist))
					for key, hop in brew_data.hop_data.items():
						f.write(
							u'database\xa7hop\xa7{name}\t{data}\n'.format(
								name=key, data=hop))
					for key, yeast in brew_data.yeast_data.items():
						f.write(
							u'database\xa7yeast\xa7{name}\t{data}\n'.format(
								name=key, data=yeast))
					for key, water_chem in brew_data.water_chemistry_additions.items():
						f.write(
							u'database\xa7water_chem\xa7{name}\t{data}\n'.format(
					...
					f.write(
						u'database\xa7constant\xa7{constants}'.format(
							constants=brew_data.constants))

	def save(self):
		u''' Save Current File if not Save As '''
	...
	def add_percent_ingredients(self, amount, curr_selection=None):
		...
		try:
			if curr_selection is None:
				...
				selection = curr_selection
			ingred_id = int(unicode(selection)[1:], 16)
			percent = self.ingredients[ingred_id - 1][u'Values'][u'Percent'] + amount
			if percent < 0:
				percent = 0
			self.ingredients[ingred_id - 1][u'Values'][u'Percent'] = percent
			self.refresh_grist()
			self.scrolled_tree_ingredient.focus_set()
			self.scrolled_tree_ingredient.see(selection)
			self.scrolled_tree_ingredient.selection_set(selection)
	...
	def add_percent_hops(self, amount, curr_selection=None):
		u''' Add a Percentage amount of Hops '''
		try:
			if curr_selection is None:
				selection = self.scrolled_tree_hops.selection()[0]
			else:
				selection = curr_selection
			hop_id = int(unicode(selection)[1:], 16)
			percent = self.hops[hop_id - 1][u'Values'][u'Percent'] + amount
			if percent < 0:
				percent = 0
			self.hops[hop_id - 1][u'Values'][u'Percent'] = percent
			self.refresh_hop()
			self.scrolled_tree_hops.focus_set()
			self.scrolled_tree_hops.see(selection)
			self.scrolled_tree_hops.selection_set(selection)
	...
	def ebu_fixed(self):
		u''' Reconfigure the hop buttons to read percentages '''
	...
	def og_fixed(self):
		u''' Reconfigure the Ingredient buttons to read percentages '''
		if self.is_ogfixed.get() == 1:
			...
			self.add_100g_ing_butt.configure(
				command=lambda: self.add_percent_ingredients(5))
			self.add_100g_ing_butt.configure(font=u"TkFixedFont")
			...
			self.add_10g_ing_butt.configure(
				command=lambda: self.add_percent_ingredients(1))
			self.add_10g_ing_butt.configure(font=u"TkFixedFont")
			...
			self.add_1g_ing_butt.configure(
				command=lambda: self.add_percent_ingredients(0.1))
			self.add_1g_ing_butt.configure(font=u"TkFixedFont")
...
class hops_editor(tk.Frame):
	u''' Editor for Hop values brew_data.hop_data '''
	...
	def widgets(self):
		...
		self.hop_panedwindow1 = tk.LabelFrame(
			width=400, text=u'Hops:', background=_bgcolor)
		self.TPanedwindow1.add(self.hop_panedwindow1)
		self.hop_panedwindow2 = tk.LabelFrame(
			text=u'Modifications:', background=_bgcolor)
		...
		self.hop_alpha_percent.place(
			relx=0.694, rely=0.283, bordermode=u'ignore')
	...
	def select_listbox(self, event):
		u''' On listbox item selection fill data values '''
		try:
	...
	def show_data(self, hop):
		u''' Show data values ready for editing '''
		self.input_state(1)
		self.name = hop
		name = hop
	...
	def input_state(self, state):
		u''' Enable or disable editable data points '''
	...
	def done(self):
		u''' Save data values to temporary variable '''
	...
	def clear_form(self):
		u''' Reset all entries to nothing '''
		self.hop_name_ent.delete(0, tk.END)
		self.hop_origin_ent.delete(0, tk.END)
		self.hop_alpha_ent.delete(0, tk.END)
		self.hop_comm_ent.delete(0, tk.END)
	...
	def delete(self):
		u''' Delete hop from data set '''
		del brew_data.hop_data[self.hop_lstbx.get(
			self.hop_lstbx.curselection())]
		self.hop_lstbx.delete(self.hop_lstbx.curselection())

	def new(self):
		u''' Add a New Hop to dataset '''
		name = u'New Hop {num}'.format(
			num=sum(u'New Hop' in s for s in brew_data.hop_data))
		self.hop_lstbx.insert(tk.END, name)
		try:
			brew_data.hop_data[name] = brew_data.hop_data[self.hop_lstbx.get(
				self.hop_lstbx.curselection())]
		except BaseException:
			try:
	...
	def save(self):
		u''' Saves current temporary variables to file '''
		with open(resource_path(u'hop_data.txt'), u'w') as f:
			for hop, value in brew_data.hop_data.items():
				name = hop
				form = value[u'Form']
				...
				description = value[u'Description']
				f.write(
					u'{name}\t{form}\t{origin}\t{alpha}\t{use}\t{description}\n'.format(
						name=name,
						form=form,
						origin=origin,
						alpha=alpha,
	...
	def reinsert(self):
		u''' Rebuild listbox with current datacollection '''
...
class grist_editor(tk.Frame):
	u''' Editor for Grist values brew_data.grist_data '''
	def __init__(self, parent):
		tk.Frame.__init__(self, parent)
	...
	def widgets(self):
		...
		self.TPanedwindow1 = tk.PanedWindow(
			self, orient=u"horizontal", background=_bgcolor)
		self.TPanedwindow1.place(
			relx=0.013,
			rely=0.0,
			relheight=0.973,
		...
		self.grist_colour_lbl.place(
			relx=0.056, rely=0.152, bordermode=u'ignore')
		...
		self.grist_extract_lbl.place(
			relx=0.056, rely=0.217, bordermode=u'ignore')
		...
		self.grist_extract_ldk.place(
			relx=0.75, rely=0.217, bordermode=u'ignore')
		...
		self.grist_moisture_lbl.place(
			relx=0.056, rely=0.283, bordermode=u'ignore')
		...
		self.grist_moisture_percent.place(
			relx=0.75, rely=0.283, bordermode=u'ignore')
		...
		self.grist_ferment_lbl.place(
			relx=0.056, rely=0.348, bordermode=u'ignore')
		...
		self.grist_ferment_percent.place(
			relx=0.75, rely=0.348, bordermode=u'ignore')
		...
		self.grist_type_combo_values.append(
			[
				grist[u'Type'] for key,
				grist in brew_data.grist_data.items() if grist[u'Type'] not in [
					1.0,
					2.0,
					3.0,
					4.0,
		...
		self.grist_cancel_butt.configure(
			command=lambda: self.show_data(
				self.grist_lstbx.get(
					tk.ACTIVE)))

		self.grist_clear_butt = tk.Button(self.grist_panedwindow2)
		self.grist_clear_butt.place(
			relx=0.389,
			rely=0.652,
			height=28,
			width=83,
		...
		self.grist_save_data_butt.place(
			relx=0.222,
			rely=0.739,
			height=108,
			width=213,
	...
	def show_data(self, grist):
		...
		grist_type = int(float(brew_data.grist_data[name][u'Type']))
	...
	def delete(self):
		u''' Delete grist from data set '''
		del brew_data.grist_data[self.grist_lstbx.get(
			self.grist_lstbx.curselection())]
		self.grist_lstbx.delete(self.grist_lstbx.curselection())

	def new(self):
		u''' Add a New Grist to dataset '''
		name = u'New Grist {num}'.format(
			num=sum(u'New Grist' in s for s in brew_data.grist_data))
		self.grist_lstbx.insert(tk.END, name)
		try:
			brew_data.grist_data[name] = brew_data.grist_data[self.grist_lstbx.get(
				self.grist_lstbx.curselection())]
		except BaseException:
			try:
	...
	def save(self):
		...
		with open(resource_path(u'grain_data.txt'), u'w') as f:
			for ingredient, value in brew_data.grist_data.items():
				...
				description = value[u'Description']
				f.write(
					u'{name}\t{ebc}\t{type}\t{extract}\t{moisture}\t{fermentability}\t{description}\n'.format(
						name=name,
						ebc=ebc,
						type=type,
						extract=extract,
...
class defaults_editor(tk.Frame):
	u''' An editor for default vals (eff) '''
	def __init__(self, parent):
	...
	def widgets(self):
		u''' Create widgets for defaults_editor '''
		_fgcolor = u'#000000'  # X11 color: 'black'
		_compcolor = u'#d9d9d9'  # X11 color: 'gray85'
		_ana1color = u'#d9d9d9'  # X11 color: 'gray85'
		_ana2color = u'#ececec'  # Closest X11 color: 'gray92'
		...
		self.liquor_to_grist_ent.place(
			relx=0.253,
			rely=0.317,
			relheight=0.044,
			relwidth=0.106)
		...
		self.mash_efficiency_ent.place(
			relx=0.215,
			rely=0.233,
			relheight=0.044,
			relwidth=0.106)
		...
		self.mash_efficiency_percent_lbl.place(
			relx=0.328, rely=0.233, height=19, width=15)
		...
		self.liquor_to_grist_lperkg_lbl.place(
			relx=0.366, rely=0.317, height=19, width=35)
		...
		self.attenuation_defaults_lbl.place(
			relx=0.038, rely=0.402, height=19, width=155)
		...
		self.attenuation_type_combo = tk.OptionMenu(
			self, self.attenuation_type_var, *self.attenuation_types)
		self.attenuation_type_combo.place(
			relx=0.253, rely=0.402, relheight=0.064, relwidth=0.16)
		...
		self.attenuation_temp_combo = tk.OptionMenu(
			self, self.attenuation_temp_var, *self.attenuation_temps)
		self.attenuation_temp_combo.place(
			relx=0.429, rely=0.402, relheight=0.064, relwidth=0.072)
		...
		self.save_on_close_combo = tk.OptionMenu(
			self, self.save_on_close_var, u'True', u'False')
		self.save_on_close_combo.place(
			relx=0.202,
			rely=0.486,
			relheight=0.064,
			relwidth=0.122)
		...
		self.default_boil_time_spinbox.place(
			relx=0.227, rely=0.571, relheight=0.049, relwidth=0.086)
		...
		self.default_boil_time_min_lbl.place(
			relx=0.328, rely=0.571, height=19, width=65)
		...
		self.replace_default_vars.configure(
			text=u'''Update Default Configuration:''')
		...
		self.replace_default_vars_chckbutt.configure(
			variable=self.replace_default_vars_variable)

	def reset_to_defaults(self):
		u''' Reset all current entries to textfile database '''
			...
			for constants in data:
				if constants[0] == u'efficiency':
					self.mash_efficiency_ent.insert(
						0, float(constants[1]))  # float(constants[1])/100
				elif constants[0] == u'volume':
					self.target_vol_ent.insert(0, float(constants[1]))
				elif constants[0] == u'evaporation':
					# (float(constants[1])/100)+1
					self.boil_vol_ent.insert(0, float(constants[1]) + 100)
				elif constants[0] == u'LGratio':
					self.liquor_to_grist_ent.insert(0, float(constants[1]))
				elif constants[0] == u'attenuation':
					atten_type = constants[1].split(u'-')[0]
					self.attenuation_type_var.set(
						atten_type if atten_type != u'med' else u'Medium')
					...
					self.replace_default_vars_variable.set(
						False if constants[1] == u'True' else True)

	def save_all(self):
		u''' Save current defaults to dataset'''
		with open(resource_path(u'defaults.txt'), u'w') as f:
			...
			evaporation = (float(self.boil_vol_ent.get()) - 100)
			...
			attenuation = (attenuation_type if attenuation_type !=
						   u'medium' else u'med') + u'-' + (attenuation_temp)
			...
			f.write(u'efficiency={efficiency}\nvolume={volume}\nevaporation={evaporation}\nLGratio={LGratio}\nattenuation={attenuation}\nsave_close={save_close}\nboil_time={boil_time}\nreplace_defaults={replace_defaults}'.format(
				efficiency=efficiency, volume=volume, evaporation=evaporation, LGratio=LGratio, attenuation=attenuation, save_close=save_close, boil_time=boil_time, replace_defaults=replace_defaults))

	def temp_save(self):
		u''' Temporarily save entry values to brew_data.constants '''
		brew_data.constants[u'Volume'] = float(self.target_vol_ent.get())
		brew_data.constants[u'Efficiency'] = float(
			self.mash_efficiency_ent.get()) / 100
		brew_data.constants[u'Boil Volume Scale'] = (
			float(self.boil_vol_ent.get()) / 100)
		brew_data.constants[u'Liquor To Grist Ratio'] = float(
			self.liquor_to_grist_ent.get())
		brew_data.constants[u'Save On Close'] = True if self.save_on_close_var.get(
		) == u'True' else False
		brew_data.constants[u'Default Boil Time'] = int(
			self.default_boil_time_spinbox.get())
		brew_data.constants[u'Replace Defaults'] = True if self.save_on_close_var.get(
		) == u'True' else False

	def open_locals(self):
		u''' Open saved textfile values into entries '''
		self.target_vol_ent.delete(0, tk.END)
		self.boil_vol_ent.delete(0, tk.END)
		self.mash_efficiency_ent.delete(0, tk.END)
		self.liquor_to_grist_ent.delete(0, tk.END)
		...
		self.mash_efficiency_ent.insert(
			0, brew_data.constants[u'Efficiency'] * 100)
		...
		self.liquor_to_grist_ent.insert(
			0, brew_data.constants[u'Liquor To Grist Ratio'])
		self.default_boil_time_spinbox.insert(
			0, brew_data.constants[u'Default Boil Time'])
		self.replace_default_vars_variable.set(
			not brew_data.constants[u'Replace Defaults'])


class special_editor(tk.Frame):
	u''' An editor+configurator for non-beerengine vals '''
	def __init__(self, parent):
		tk.Frame.__init__(self, parent, background=_bgcolor)
		...
		self.current_attenuation.set(
			brew_data.constants[u'Attenuation Default'])
		self.original_additions = list(sorted(
			brew_data.water_chemistry_additions)) + list(sorted(brew_data.yeast_data))
	...
	def widgets(self):
		u''' Build widgets for the Experimental Attenuation Editor '''
		_fgcolor = u'#000000'  # X11 color: 'black'
		_compcolor = u'#d9d9d9'  # X11 color: 'gray85'
		_ana1color = u'#d9d9d9'  # X11 color: 'gray85'
		_ana2color = u'#ececec'  # Closest X11 color: 'gray92'
		...
		self.attenuation_low_lbl.place(
			relx=0.278,
			rely=0.109,
			height=19,
			width=28,
		...
		self.attenuation_med_lbl.place(
			relx=0.5,
			rely=0.109,
			height=19,
			width=30,
		...
		self.attenuation_high_lbl.place(
			relx=0.722,
			rely=0.109,
			height=19,
			width=42,
		...
		self.attenuation_62_degrees.place(
			relx=0.056,
			rely=0.182,
			height=19,
			width=34,
		...
		self.attenuation_63_degrees.place(
			relx=0.056,
			rely=0.255,
			height=19,
			width=34,
		...
		self.attenuation_64_degrees.place(
			relx=0.056,
			rely=0.327,
			height=19,
			width=34,
		...
		self.attenuation_65_degrees.place(
			relx=0.056, rely=0.4, height=19, width=34, bordermode=u'ignore')
		...
		self.attenuation_66_degrees.place(
			relx=0.056,
			rely=0.473,
			height=19,
			width=34,
		...
		self.attenuation_67_degrees.place(
			relx=0.056,
			rely=0.545,
			height=19,
			width=34,
		...
		self.attenuation_68_degrees.place(
			relx=0.056,
			rely=0.618,
			height=19,
			width=34,
		...
		self.attenuation_69_degrees.place(
			relx=0.056,
			rely=0.691,
			height=19,
			width=34,
		...
		self.attenuation_70_degrees.place(
			relx=0.056,
			rely=0.764,
			height=19,
			width=34,
		...
		self.attenuation_71_degrees.place(
			relx=0.056,
			rely=0.836,
			height=19,
			width=34,
		...
		self.attenuation_72_degrees.place(
			relx=0.056,
			rely=0.909,
			height=19,
			width=34,
		...
		self.attenuation_high_62.place(
			relx=0.177,
			rely=0.127,
			relheight=0.049,
			relwidth=0.039)
		...
		self.attenuation_high_63.place(
			relx=0.177,
			rely=0.169,
			relheight=0.049,
			relwidth=0.039)
		...
		self.attenuation_high_64.place(
			relx=0.177,
			rely=0.211,
			relheight=0.049,
			relwidth=0.039)
		...
		self.attenuation_high_65.place(
			relx=0.177,
			rely=0.254,
			relheight=0.049,
			relwidth=0.039)
		...
		self.attenuation_high_66.place(
			relx=0.177,
			rely=0.296,
			relheight=0.049,
			relwidth=0.039)
		...
		self.attenuation_high_67.place(
			relx=0.177,
			rely=0.338,
			relheight=0.049,
			relwidth=0.039)
		...
		self.attenuation_high_68.place(
			relx=0.177,
			rely=0.381,
			relheight=0.049,
			relwidth=0.039)
		...
		self.attenuation_high_69.place(
			relx=0.177,
			rely=0.423,
			relheight=0.049,
			relwidth=0.039)
		...
		self.attenuation_high_70.place(
			relx=0.177,
			rely=0.465,
			relheight=0.049,
			relwidth=0.039)
		...
		self.attenuation_high_71.place(
			relx=0.177,
			rely=0.507,
			relheight=0.049,
			relwidth=0.039)
		...
		self.attenuation_high_72.place(
			relx=0.177, rely=0.55, relheight=0.049, relwidth=0.039)
		...
		# , height=28, width=53  relx=0.505, rely=0.317
		self.move_all_left.place(relx=0.505, rely=0.402, height=28, width=53)
		...
		self.water_boil_time_spinbx = tk.Spinbox(
			self.water_boil_frame, from_=1.0, to=9999.0)
		self.water_boil_time_spinbx.place(
			relx=0.444,
			rely=0.4,
			relheight=0.307,
			relwidth=0.322,
	...
	def refresh_orig(self):
		u''' Refresh the original additions listbox '''
	...
	def refresh_add(self):
		u''' Refresh the added additions listbox '''
		...
		for idx, addition in enumerate(self.added_additions):
			...
			if addition in brew_data.yeast_data:
				self.water_chem_added_lstbx.itemconfig(
					idx, {u'bg': u'lightblue'})

	def refresh_all(self):
		u''' Refresh all listboxes '''
		self.refresh_orig()
		self.refresh_add()

	def move_one_left_right(self):
		u''' Move selection from left listbox to right listbox '''
		try:
	...
	def move_all_left_right(self):
		u''' Move all from left listbox to right listbox '''
	...
	def move_one_right_left(self):
		u''' Move selection from right listbox to left listbox '''
		try:
	...
	def move_all_right_left(self):
		u''' Move all from right listbox to left listbox '''
	...
	def water_boil_check(self):
		u''' Enable/disable water boil time entry '''
	...
	def new_water_chem(self):
		u'''  Create New Water Chemistry Addition '''
		def on_type_change(*args):
			u''' When type changes enable/disable features '''
		...
		def done():
			u'''  Add New Water Chemistry Addition '''
			brew_data.water_chemistry_additions[name_var.get()] = {u'Values': {
				u'Type': type_var.get()}}
			if type_var.get() == u'Hop':
				brew_data.water_chemistry_additions[name_var.get(
				)][u'Values'][u'Time'] = float(time_var.get())
			self.original_additions = list(set(sorted(brew_data.water_chemistry_additions)) - set(
				self.added_additions)) + list((set(sorted(brew_data.yeast_data)) - set(self.added_additions)))
			self.refresh_all()
		...
		def save_to_database():
			u''' Add New Water Chemistry Addition to textfile database '''
			done()
			with open(resource_path(u'water_chem_data.txt'), u'w') as f:
				for water_chem, values in brew_data.water_chemistry_additions.items():
					value = values[u'Values']
		...
		name_var = tk.StringVar(value=u'New Item {num}'.format(
			num=sum(u'New Item' in s for s in brew_data.water_chemistry_additions)))
		time_var = tk.IntVar()
		...
		tk.Button(
			new_water_chem_win,
			text=u"Save To Database",
			command=save_to_database).grid(
			row=4,
			column=0,
			columnspan=2,
			sticky=u'nsew')
...
class yeast_editor(tk.Frame):
	u''' Editor for Yeast values brew_data.yeast_data '''
	def __init__(self, parent):
		tk.Frame.__init__(self, parent)
	...
	def widgets(self):
		u''' Create widgets for Yeast Editor '''
		_fgcolor = u'#000000'  # X11 color: 'black'
		_compcolor = u'#d9d9d9'  # X11 color: 'gray85'
		_ana1color = u'#d9d9d9'  # X11 color: 'gray85'
		_ana2color = u'#ececec'  # Closest X11 color: 'gray92'
		...
		self.yeast_flocc_combo_values = [
			u'Low', u'Low/Medium', u'Medium', u'Medium/High', u'High']
		...
		self.yeast_attenuation_lbl.place(
			relx=0.056, rely=0.413, bordermode=u'ignore')
		...
		self.yeast_temperature_lbl.place(
			relx=0.056,
			rely=0.478,
			height=19,
			width=110,
		...
		self.yeast_attenuation_ent.place(
			relx=0.333,
			rely=0.413,
			relheight=0.046,
			relwidth=0.4,
		...
		self.yeast_temperature_spinbox1 = tk.Spinbox(
			self.yeast_panedwindow2, from_=1.0, to=100.0)
		self.yeast_temperature_spinbox1_value = tk.DoubleVar()
		self.yeast_temperature_spinbox1.place(
			relx=0.389,
			rely=0.478,
			relheight=0.05,
			relwidth=0.133,
		...
		self.yeast_temperature_spinbox1.configure(
			textvariable=self.yeast_temperature_spinbox1_value)

		self.yeast_temperature_spinbox2 = tk.Spinbox(
			self.yeast_panedwindow2, from_=1.0, to=100.0)
		self.yeast_temperature_spinbox2_value = tk.DoubleVar()
		self.yeast_temperature_spinbox2.place(
			relx=0.528,
			rely=0.478,
			relheight=0.05,
			relwidth=0.133,
		...
		self.yeast_temperature_spinbox2.configure(
			textvariable=self.yeast_temperature_spinbox2_value)
		...
		self.yeast_cancel_butt.configure(
			command=lambda: self.show_data(
				self.yeast_lstbx.get(
					tk.ACTIVE)))

		self.yeast_clear_butt = tk.Button(self.yeast_panedwindow2)
		self.yeast_clear_butt.place(
			relx=0.389,
			rely=0.652,
			height=28,
			width=83,
		...
		self.yeast_save_data_butt.place(
			relx=0.222,
			rely=0.739,
			height=108,
			width=213,
	...
	def show_data(self, yeast):
		...
		if brew_data.yeast_data[name][u'Temperature'] != u'Unknown':
			temperature1 = float(
				brew_data.yeast_data[name][u'Temperature'].replace(
					u'°', u'').split(u'-')[0])
			temperature2 = float(
				brew_data.yeast_data[name][u'Temperature'].replace(
					u'°', u'').split(u'-')[1])
	...
	def new(self):
		u''' Add a New Yeast to dataset '''
		name = u'New Yeast {num}'.format(
			num=sum(u'New Yeast' in s for s in brew_data.yeast_data))
		self.yeast_lstbx.insert(tk.END, name)
		try:
			brew_data.yeast_data[name] = brew_data.yeast_data[self.yeast_lstbx.get(
				self.yeast_lstbx.curselection())]
		except BaseException:
			try:
				...
				brew_data.yeast_data[name] = {
					u'Type': u'D',
					u'Lab': u'Lallemand',
					u'Flocculation': u'Low',
					u'Attenuation': u'High',
					u'Temperature': u'66-72°',
...
class notes_area(tk.Frame):
	u''' A Notes Area salved from old Texpert Project '''
	def __init__(self, parent):
		tk.Frame.__init__(self, parent)
	...
	def widgets(self):
		u''' Create the window widgets + menu items '''
		#######################################################################
		#				Salvaged from https://github.com/jimbob88/texpert/					   #
		#######################################################################
		self.texpert = ScrolledText(
			self,
			bg=u"white",
			undo=True,
			maxundo=-1,
		...
		self.editmenu.add_command(
			...
			accelerator=u"Ctrl+Z")
		self.editmenu.add_command(
			label=u"Redo",
			command=self.redo_com,
			accelerator=u"Shift+Ctrl+Z")
		self.editmenu.add_separator()
		self.editmenu.add_command(
			label=u"Cut",
			command=self.cut_com,
			accelerator=u"Ctrl+X")
	...
	def undo_com(self):
		...
		try:
			self.texpert.event_generate(u"<<Undo>>")
		except tk.TclError:
			print u'Undo Failed'
	...
	def redo_com(self):
		u''' Redo Command '''
		try:
			self.texpert.event_generate(u"<<Redo>>")
		except tk.TclError:
			print u'Redo Failed'

	def cut_com(self):
		u''' Cut command (Ctrl-x) '''
		try:
			self.texpert.event_generate(u"<<Cut>>")
		except tk.TclError:
			pass

	def copy_com(self):
		u''' Copy command (Ctrl-C) '''
		try:
			self.texpert.event_generate(u"<<Copy>>")
		except tk.TclError:
			pass

	def paste_com(self):
		u''' Paste command (Ctrl-V) '''
		try:
			self.texpert.event_generate(u"<<Paste>>")
		except tk.TclError:
			pass

	def select_all(self, event=None):
		u''' Select all in textbox '''
...
def _create_container(func):
	...
	def wrapped(cls, master, **kw):
		...
		container.bind(
			u'<Leave>',
			lambda e: _unbound_to_mousewheel(
				e,
				container))
...
class ScrolledTreeView(AutoScroll, ttk.Treeview):
	...
	def insert(self, parent, index, iid=None, **kw):
		...
		if iid is not None:
			...
			iid = u'I{iid}'.format(iid=format(
				len(self.get_children()) + 1, u'03x'))
			res = self.tk.call(self._w, u"insert", parent, index,
							   u"-id", iid, *opts)
		return res
...
def _unbound_to_mousewheel(event, widget):
	u''' Disable On mousewheel scroll move scrollbar '''
	if platform.system() == u'Windows' or platform.system() == u'Darwin':
...
def _on_mousewheel(event, widget):
	u''' On mousewheel scroll move scrollbar '''
...
def _on_shiftmouse(event, widget):
	u''' On shift mousewheel scroll move scrollbar '''
	if platform.system() == u'Windows':
...
def resource_path(relative_path):
	...
	if __mode__ == u'local':
		...
		if os.path.basename(relative_path) == u'logo.png':
			path = u'/usr/include/wheelers-wort-works/logo.png'
		elif os.path.splitext(os.path.basename(relative_path))[1] == u'.html':
			path = os.path.join(
				os.path.expanduser(u'~/.config/Wheelers-Wort-Works/recipes/html'),
				relative_path)
		path = os.path.join(
			os.path.expanduser(u'~/.config/Wheelers-Wort-Works/'),
			relative_path)
	return path
...
def main(file=None, update_available=False):
	u''' Main function - launches tk interface '''
	root = tk.Tk()
	...
	if update_available:
		...
		command = u"{command}".format(
			command=(
				u'sudo wheelers-wort-works --coreupdate' if __mode__ == u'deb' else u'python3 main.py --coreupdate'))
