...
class beer_engine_mainwin(object):
	...
	def zero_ingredients(self, curr_selection=None):
		...
		try:
			...
			id = int(unicode(selection)[1:], 16)
			EBC = int(
				brew_data.grist_data[self.ingredients[id - 1][u'Name']][u'EBC'])
			self.ingredients[id - 1] = {u'Name': self.ingredients[id - 1][u'Name'], u'Values': {
				u'EBC': EBC, u'Grav': 0.0, u'lb:oz': (0.0, 0.0), u'Grams': 0, u'Percent': 0.0}}
			self.refresh_grist()
			self.recalculate()
			self.scrolled_tree_ingredient.focus_set()
	...
	def zero_hops(self):
		...
		try:
			...
			self.hops[id - 1] = {u'Name': self.hops[id - 1][u'Name'],
								 u'Values': {u'Type': type,
											u'Alpha': alpha,
											u'Time': 0.0,
											u'Util': 0.0,
	...
	def open_file(self, file):
		...
		if file != u'' and file is not None and not isinstance(file, tuple):
			...
			self.seventh_tab.texpert.insert(
				u'1.0', ast.literal_eval(u"'" + notes + u"'"))
			self.refresh_hop()
			self.refresh_grist()
	...
	def save(self):
		...
		with open(resource_path(u'hop_data.txt'), u'w') as f:
			for hop, value in brew_data.hop_data.items():
				...
				description = value[u'Description']
				f.write(
					u'{name}\t{type}\t{origin}\t{alpha}\t{use}\t{description}\n'.format(
						name=name,
						type=type,
						origin=origin,
						alpha=alpha,
			...
			for constants in data:
				if constants[0] == u'efficiency':
					...
					self.attenuation_type_var.set(
						type if type != u'med' else u'Medium')
...
def main(file=None, update_available=False):
	...
	if update_available:
		messagebox.showinfo(
			u"Update Available",
			u"An update has become available, it is recommended you run the command: {command}".format(
				command=(
