...
def get_wiki(section, name):
	if section == u'engineroom': section = u'An-Advanced-Guide-to-the-Engine-Room'
	elif section == u'ingrededitors': section = u'An-Advanced-Guide-to-the-Hop⁄Grist⁄Yeast-Editor'
	elif section == u'defaults': section = u'An-Advanced-Guide-to-the-Defaults-Editor'
	elif section == u'attenuation': section = u'An-Advanced-Guide-to-the-Experimental-Attenuation-Tab'
	if not name.startswith(u'#'): name = u'#' + name
	return u'https://github.com/jimbob88/wheelers-wort-works/wiki/' + section + name
...
class help_button(tk.Button):
	def __init__(self, master, url, *args, **kwargs):
		...
		self.help_url = url
		self.orig_command = kwargs[u'command']
...
class help_checkbutton(tk.Checkbutton):
	...
	def toggle_help(self):
		if self.help_mode:
			self.unbind(u'<Button-1>', self.bind_com)
			self[u'command'] = self.orig_command
			self.configure(cursor=u"")
		else:
			self.orig_command = self[u'command']
			self[u'command'] = u''
			self.bind_com = self.bind(u'<Button-1>', lambda e: self.get_help())
			self.configure(cursor=u"question_arrow")
...
class help_label(tk.Label):
	...
	def toggle_help(self):
		if self.help_mode:
			...
			self.bind_com = self.bind(u'<Button-1>', lambda e: webbrowser.open_new_tab(self.help_url))
			self.configure(cursor=u"question_arrow")
		self.help_mode = not self.help_mode
...
class beer_engine_mainwin(object):
	def __init__(self, master=None):
		...
		self.master.geometry(u"+674+369")
		...
		self.help_menu.add_checkbutton(activebackground=u"#ececec",
				activeforeground=u"#000000",
				background=_bgcolor,
				font=u"TkMenuFont",
				foreground=u"#000000",
		...
		self.recipe_name_lbl = help_label(self.first_tab,
										  url=get_wiki(u'engineroom', u'recipe-name'))
		...
		self.volume_lbl = help_label(self.first_tab,
									 url=get_wiki(u'engineroom', u'volume'))
		...
		self.boil_volume_lbl = help_label(self.first_tab,
										  url=get_wiki(u'engineroom', u'boil-volume'))
		...
		self.ingredient_rem_butt =help_button(self.first_tab,
											 url=get_wiki(u'engineroom', u'remove'),
		...
		self.ingredient_add_new_butt = help_button(self.first_tab,
												   url=get_wiki(u'engineroom', u'add-new-button'),
		...
		self.add_1000g_ing_butt = help_button(self.first_tab,
											 url=get_wiki(u'engineroom', u'adjust-weight'),
		...
		self.original_gravity_lbl = help_label(self.first_tab,
										  	   url=get_wiki(u'engineroom', u'original-gravity'))
		...
		self.ingredient_zero_butt = help_button(self.first_tab,
											  url=get_wiki(u'engineroom', u'zero'),
		...
		self.recalc_butt = help_button(self.first_tab,
									 url=get_wiki(u'engineroom', u'the-recalculate-button'),
		...
		self.hop_add_new_butt = help_button(self.first_tab,
											  url=get_wiki(u'engineroom', u'add-hop-button'),
		...
		self.adjust_weight_hop_lbl = help_label(self.first_tab,
										  url=get_wiki(u'engineroom', u'adjust-weight-1'))
		...
		self.hop_zero_butt = help_button(self.first_tab,
										 url=get_wiki(u'engineroom', u'zero-1'),
		...
		self.bitterness_ibu_lbl = help_label(self.first_tab,
										     url=get_wiki(u'engineroom', u'bitterness-ibus'))
		...
		self.hop_rem_butt = help_button(self.first_tab,
									url=get_wiki(u'engineroom', u'the-quit-button'),
		...
		self.add_time_butt_1 = help_button(self.first_tab,
										   url=get_wiki(u'engineroom', u'time-increasedecrease'),
		...
		self.add_alpha_butt_pt1 = help_button(self.first_tab,
										   url=get_wiki(u'engineroom', u'alpha-increasedecrease'),
		...
		self.ingredients_imperial_chk_butt = help_checkbutton(self.first_tab,
															  url=get_wiki(u'engineroom', u'using-imperial-units'))
		...
		self.hops_imperial_chk_butt = help_checkbutton(self.first_tab,
														url=get_wiki(u'engineroom', u'using-imperial-units-1'))
		...
		self.ogfixed_chkbutton = help_checkbutton(self.first_tab,
												  url=get_wiki(u'engineroom', u'fixed-original-gravity'))
		...
		self.ebufixed_chkbutton = help_checkbutton(self.first_tab,
												   url=get_wiki(u'engineroom', u'fixed-bitterness-ibus'))
		...
		def make_treeview():
			if u'scrolled_tree_ingredient' in vars(self):
				...
				self.scrolled_tree_ingredient = ScrolledTreeView(self.frame_ingredients,
																 url=get_wiki(u'engineroom', u'fermentable-ingredients-table'),
																 style=u"mystyle.Treeview")
		...
		def make_treeview():
			if u'scrolled_tree_hops' in vars(self):
				...
				self.scrolled_tree_hops = ScrolledTreeView(self.frame_hops,
														   url=get_wiki(u'engineroom', u'hop-varieties-table'),
														   style=u"mystyle.Treeview")
	...
	def add_grist(self):
		...
		grist_options = ScrolledTreeView(add_grist_gui, url=get_wiki(u'engineroom', u'the-dialogue')
														, show=u"tree", columns=(u"EBC"))
	...
	def add_hop(self):
		...
		hop_options = ScrolledTreeView(add_hop_gui, url=get_wiki(u'engineroom', u'the-dialogue-1'), show=u"tree", columns=(u"Form"))
	...
	def open_file(self, file):
		if file != u'' and file != None and type(file) != tuple:
			...
			self.seventh_tab.texpert.insert(u'1.0', ast.literal_eval(u"'''"+notes.replace(u"'", ur'\'').replace(u'"', ur'\"')+u"'''"))
			self.refresh_hop()
			self.refresh_grist()
	...
	def database_to_folder(self, folder=None):
		if folder == None: 
			if not os.path.exists(os.path.join(os.path.dirname(resource_path(u'hop_data.txt')), u'backups')):
				try:
					os.makedirs(os.path.join(os.path.dirname(resource_path(u'hop_data.txt')), u'backups'))
					folder = os.path.join(os.path.dirname(resource_path(u'hop_data.txt')), u'backups')
				except:
					folder = os.path.dirname(resource_path(u'hop_data.txt')) 
			else:
				folder = os.path.join(os.path.dirname(resource_path(u'hop_data.txt')), u'backups')
		...
		messagebox.showinfo(u'Backup Successful', u'Backup {time} Created Successfully'.format(time=curr_time))		

	def restore_backup_dialogue(self):
		def restore(folder=None):
			if folder == None: 
				if os.path.exists(os.path.join(os.path.dirname(resource_path(u'hop_data.txt')), u'backups')):
					folder = os.path.join(os.path.dirname(resource_path(u'hop_data.txt')), u'backups')
				else:
					folder = os.path.dirname(resource_path(u'hop_data.txt'))
			file_starts = [u'grain_data', u'hop_data', u'water_chem_data', u'yeast_data', u'defaults']
			try:			
				...
				messagebox.showwarning(u'No Backup Selected', u'Please select a Restore Point', parent=restore_backup_dia)
				return
			if messagebox.askquestion(u'Verify', u'This operation will overwrite current database. Are you sure you wish to Restore Database?', parent=restore_backup_dia) == u'no':
				return
			try:	
				time = backup_list[id-1]
				for file_start in file_starts:
					print u'{file_start} {time}.txt'.format(file_start=file_start, time=time)
					if os.path.isfile(os.path.join(folder, u'{file_start} {time}.txt'.format(file_start=file_start, time=time))):
						with open(resource_path(file_start+u'.txt'), u'w') as f:
							for line in open(os.path.join(folder, u'{file_start} {time}.txt'.format(file_start=file_start, time=time))).readlines():
								f.write(line)
					else:
						messagebox.showerror(u'File Not Found', u'File Not Found: {file}'.format(file=os.path.join(folder, u'{file_start} {time}.txt'.format(file_start=file_start, time=time))))
				messagebox.showinfo(u'Restore Successful', u'Backup {time} Restored Successfully'.format(time=time), parent=restore_backup_dia)
			except Exception, e:
				print e
		...
		backups = ScrolledTreeView(restore_backup_dia, url=u'', custom_insert=False, show=u"tree")
		backups.grid(row=0,column=0)
		if os.path.isfile(resource_path(u'backups.txt')):
			backup_list = [backup.strip() for backup in open(resource_path(u'backups.txt')).readlines()]
		else:
			backup_list = []
		for backup in backup_list:
			curr_insert = backups.insert(u'', tk.END, text=(backup))
			print curr_insert
			for idx, file_start in enumerate([u'grain_data', u'hop_data', u'water_chem_data', u'yeast_data', u'defaults']):
				if os.path.exists(os.path.join(os.path.dirname(resource_path(u'hop_data.txt')), u'backups')):
					file_name = os.path.join(os.path.dirname(resource_path(u'hop_data.txt')), u'backups/{file_start} {time}.txt'.format(file_start=file_start, time=backup))
				else:
					file_name = resource_path(u'{file_start} {time}.txt'.format(file_start=file_start, time=backup))
				print file_name
				if os.path.isfile(file_name):
					print idx
					backups.insert(curr_insert, u'end', text=u'{file_start}.txt'.format(file_start=file_start), iid=u'{file_start}{iid}'.format(file_start=file_start, iid=curr_insert))
...
class hops_editor(tk.Frame):
	...
	def widgets(self):
		...
		self.hop_panedwindow1 = tk.LabelFrame(self, text=u'Hops:', background=_bgcolor)
		self.hop_panedwindow1.place(relx=0.005, rely=0.005, relheight=0.990, relwidth=0.5)

		self.hop_panedwindow2 = tk.LabelFrame(self, text=u'Modifications:', background=_bgcolor)
		...
		self.hop_delete_butt = help_button(self.hop_panedwindow1,
										   url=get_wiki(u'ingrededitors', u'the-delete-button'),
										   command=self.delete)
		...
		self.hop_modify_butt = help_button(self.hop_panedwindow1,
										   url=get_wiki(u'ingrededitors', u'the-modify-button'),
		...
		self.hop_new_butt = help_button(self.hop_panedwindow1,
										url=get_wiki(u'ingrededitors', u'the-new-button'),
										command=self.new)
		...
		self.hop_comm_lbl = help_label(self.hop_panedwindow2,
									   url=get_wiki(u'ingrededitors', u'the-comments-entry'))
		...
		self.hop_cancel_butt = help_button(self.hop_panedwindow2,
											url=get_wiki(u'ingrededitors', u'the-cancel-button'),
		...
		self.hop_clear_butt = help_button(self.hop_panedwindow2,
										  url=get_wiki(u'ingrededitors', u'the-clear-form-button'),
		...
		self.hop_done_butt = help_button(self.hop_panedwindow2,
										 url=get_wiki(u'ingrededitors', u'the-done-button'),
										 command=self.done)
		...
		self.hop_save_data_butt = help_button(self.hop_panedwindow2,
											  url=get_wiki(u'ingrededitors', u'the-save-to-database-button'),
											  command=self.save)
		self.hop_save_data_butt.place(relx=0.222, rely=0.696, relwidth=0.5713
				, relheight=0.2312, bordermode=u'ignore')
...
class grist_editor(tk.Frame):
	...
	def widgets(self):
		...
		self.grist_panedwindow1 = tk.LabelFrame(self, text=u'Fermentable Ingredients:', background=_bgcolor)
		self.grist_panedwindow1.place(relx=0.005, rely=0.005, relheight=0.990, relwidth=0.5)
		self.grist_panedwindow2 = tk.LabelFrame(self, text=u'Modifications:', background=_bgcolor)
		...
		self.grist_comm_lbl = help_label(self.grist_panedwindow2,
									   url=get_wiki(u'ingrededitors', u''))
...
class defaults_editor(tk.Frame):
	...
	def widgets(self):
		...
		self.target_vol_lbl = help_label(self,
										 url=get_wiki(u'defaults', u'target-volume'))
		...
		self.boil_vol_lbl = help_label(self,
									   url=get_wiki(u'defaults', u'boil-volume-scale'))
		...
		self.liquor_to_grist_lbl = help_label(self,
											  url=get_wiki(u'defaults', u'liquor-to-grist-ratio'))
		...
		self.mash_efficiency_lbl = help_label(self,
											  url=get_wiki(u'defaults', u'mash-efficiency'))
		...
		self.save_all_butt = help_button(self,
										 url=get_wiki(u'defaults', u'save-all-as-defaults'),
										 command=self.save_all)
		...
		self.done_button = help_button(self,
									   url=get_wiki(u'defaults', u'done'),
		...
		self.reset_to_defaults_butt = help_button(self,
									   			  url=get_wiki(u'defaults', u'reset-to-local-database'),
		...
		self.attenuation_defaults_lbl= help_label(self,
												  url=get_wiki(u'defaults', u'attenuation-default'))
		...
		self.save_on_close_lbl = help_label(self,
											url=get_wiki(u'defaults', u'save-on-close'))
		...
		self.default_boil_time_lbl = help_label(self,
												url=get_wiki(u'defaults', u'default-boil-time'))
...
class special_editor(tk.Frame):
	...
	def widgets(self):
		...
		self.attenuation_frame = help_labelframe(self,
												 url=get_wiki(u'attenuation', u'the-yeast-attenuation-pane'))
		...
		self.water_chem_add_frame = help_labelframe(self,
													url=get_wiki(u'attenuation', u'water-chemistry-additions-pane'))
		...
		self.water_boil_frame = help_labelframe(self,
												url=get_wiki(u'attenuation', u'water-boil-pane'))
...
class yeast_editor(tk.Frame):
	...
	def widgets(self):
		...
		# 		, relwidth=0.966)
		self.yeast_panedwindow1 = tk.LabelFrame(self, text=u'Yeasts:', background=_bgcolor)
		self.yeast_panedwindow1.place(relx=0.005, rely=0.005, relheight=0.990, relwidth=0.5)
		self.yeast_panedwindow2 = tk.LabelFrame(self, text=u'Modifications:', background=_bgcolor)
	...
	def insert(self, parent, index, iid=None, **kw):
		...
		if iid is not None:
			...
			if self.custom_insert:
				...
				res = self.tk.call(self._w, u"insert", parent, index, *opts)
		return res
...
def main(file=None, update_available=False):
	...
	if update_available:		
		...
		editmenu.add_command(
			label=u"Copy",
			command=lambda: command_box.event_generate(u"<<Copy>>"),
			accelerator=u"Ctrl+C")
		command_box.bind(u"<Control-Key-c>", lambda event: command_box.event_generate(u"<<Copy>>"))
