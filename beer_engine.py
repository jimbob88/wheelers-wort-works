...
class beer_engine_mainwin:
	...
	def __init__(self, master=None):
		...
		if not platform.system() == 'Darwin':
			self.master.tk.call(
				'wm', 'iconphoto', self.master._w, tk.PhotoImage(
	...
	def refresh_grist(self):
		...
		def refresh_indiv_grav():
			...
			volume = float(self.volume.get())
			for ingredient in self.ingredients:
				points = brew_data.grist_data[ingredient['Name']
											  ]['Extract'] * (ingredient['Values']['Grams']) / 1000
				eff = (1 if brew_data.grist_data[ingredient['Name']]['Type']
					   in non_mashables else brew_data.constants['Efficiency'])
		...
		if not self.is_ogfixed.get():
			...
			non_mashables = [6.0, 5.0]
			factor = sum([ingredient['Values']['Percent'] * brew_data.grist_data[ingredient['Name']]['Extract'] * (1 if brew_data.grist_data[ingredient['Name']]
																												   ['Type'] in non_mashables else brew_data.constants['Efficiency']) for idx, ingredient in enumerate(self.ingredients)])
			...
			for idx, ingredient in enumerate(self.ingredients):
				...
				self.ingredients[idx] = {
					'Name': ingredient['Name'], 'Values': {
						'EBC': EBC, 'Grav': 0.0, 'lb:oz': (
							lb, oz), 'Grams': weight, 'Percent': percent}}
			refresh_percentage()

		refresh_indiv_grav()
		for ingredient in self.ingredients:
			values = (
				ingredient['Values']['EBC'], round(
					ingredient['Values']['Grav'], 1), '{lb}:{oz}'.format(
					lb=int(
						ingredient['Values']['lb:oz'][0]), oz=round(
						ingredient['Values']['lb:oz'][1], 1)), round(
						ingredient['Values']['Grams'], 1), ingredient['Values']['Percent'])
	...
	def refresh_hop(self):
		...
		def make_treeview():
			...
			if 'scrolled_tree_hops' in vars(self):
				...
				self.scrolled_tree_hops.heading(
					"#0", text="Hop Variety", command=lambda: self.sort_by_hop("Hop Variety"))
								...
								column, width=max(
									[
										len(
											'{lb}:{oz}'.format(
												lb=int(
													hop['Values']['lb:oz'][0]), oz=round(
													hop['Values']['lb:oz'][1], 1))) for hop in self.hops]) * 7, anchor="center")
						else:
							self.scrolled_tree_hops.column(
								column, width=40, anchor="center")
		...
		def refresh_indiv_ibu():
			...
			for hop in self.hops:
				ibu = (hop['Values']['Grams'] * hop['Values']['Alpha'] * \
					   hop['Values']['Util']) / (float(self.boil_vol.get()) * 10)
				hop['Values']['ibu'] = ibu
		...
		if not self.is_ebufixed.get():
			...
			factor = sum([hop['Values']['Percent'] * hop['Values']['Alpha']
						  * hop['Values']['Util'] for idx, hop in enumerate(self.hops)])
		...
		for hop in self.hops:
			values = (
				hop['Values']['Type'], hop['Values']['Alpha'], hop['Values']['Time'], round(
					hop['Values']['Util'], 1), round(
					hop['Values']['ibu']), '{lb}:{oz}'.format(
					lb=int(
						hop['Values']['lb:oz'][0]), oz=round(
						hop['Values']['lb:oz'][1], 1)), round(
							hop['Values']['Grams'], 1), hop['Values']['Percent'])
	...
	def add_grist(self):
		...
		def insert():
			...
			self.ingredients.append({'Name': name, 'Values': {
	...
	def add_hop(self):
		...
		for hop in sorted(brew_data.hop_data):
			hop_options.insert(
				'', tk.END, text=(hop), values=(
					brew_data.hop_data[hop]['Form'],))
	...
	def recalculate(self):
		...
		def final_gravity():
			...
			non_mashables = [6.0, 5.0]
			a = sum([((self.attenuation_apply(ingredient) / 100) * ((ingredient['Values']['Grams'] / 1000) * brew_data.grist_data[ingredient['Name']]['Extract']))
					 * (1 if brew_data.grist_data[ingredient['Name']]['Type'] in non_mashables else brew_data.constants['Efficiency']) for ingredient in self.ingredients])
			b = sum([(((100 - self.attenuation_apply(ingredient)) / 100) * ((ingredient['Values']['Grams'] / 1000) * brew_data.grist_data[ingredient['Name']]['Extract']))
					 * (1 if brew_data.grist_data[ingredient['Name']]['Type'] in non_mashables else brew_data.constants['Efficiency']) for ingredient in self.ingredients])
		...
		def mash_liquor():
			...
			grist_mass = sum([0 if brew_data.grist_data[ingredient['Name']]['Type']
							  in non_mashables else ingredient['Values']['Grams'] for ingredient in self.ingredients]) / 1000
		...
		def colour_ebc():
			...
			non_mashables = [6.0, 5.0]
			return (sum([formula(ingredient, 1) if brew_data.grist_data[ingredient['Name']]['Type'] in non_mashables else formula(
				ingredient, brew_data.constants['Efficiency']) for ingredient in self.ingredients]))
	...
	def ingredient_to_imperial(self):
		...
		if self.is_imperial_ingredient.get() == 0:
			...
			self.rem_1000g_ing_butt.configure(
				command=lambda: self.add_weight_ingredients(-brew_data.constants['Conversion']['lb-g']))
			self.rem_1000g_ing_butt.configure(font=(None, 7))

			self.rem_100g_ing_butt.configure(text='''-1oz''')
			self.rem_100g_ing_butt.configure(
				command=lambda: self.add_weight_ingredients(-brew_data.constants['Conversion']['oz-g']))
	...
	def hop_to_imperial(self):
		...
		if self.is_imperial_hop.get() == 0:
			...
			self.rem_100g_hop_butt.configure(
				command=lambda: self.add_weight_hops(-brew_data.constants['Conversion']['oz-g'] * 4))
			...
			self.rem_25g_hop_butt.configure(
				command=lambda: self.add_weight_hops(-brew_data.constants['Conversion']['oz-g']))
			...
			self.rem_1g_hop_butt.configure(
				command=lambda: self.add_weight_hops(-brew_data.constants['Conversion']['oz-g'] / 16))
	...
	def attenuation_apply(self, ingredient):
		...
		if int(brew_data.grist_data[ingredient['Name']]
			   ['Fermentability']) == 200:
			table_dict = {
	...
	def sort_by_grist(self, column):
		...
		if column == 'Fermentable Ingredient':
			...
			self.ingredients = sorted(
				self.ingredients,
				key=lambda k: k['Values']['Grams'])
		elif column == '%':
			self.ingredients = sorted(
				self.ingredients,
				key=lambda k: k['Values']['Percent'])
	...
	def create_html(self, start='', open_browser=True, use_sorttable=False):
		...
		start += '<html><head><title>{name}</title><link rel="shortcut icon" href="{logo}" />'.format(
			name=self.recipe_name_ent.get().replace('&', '&amp;'), logo=resource_path('logo.png'))
		start += r'''
		...
		for addition in self.sixth_tab.added_additions:
			try:
				if brew_data.water_chemistry_additions[addition]['Values']['Type'] == 'Malt':
					start += '<tr>'
					start += '<td class="ing1">{name}</td>'.format(
						name=addition)
		...
		for ingredient in self.ingredients:
			...
			start += '<td class="ing3">{grams}</td>'.format(
				grams=(
					round(
						ingredient['Values']['Grams'], 1)) if (
					ingredient['Values']['Grams'] - int(
						ingredient['Values']['Grams'])) >= 2 else round(
					ingredient['Values']['Grams']))
			start += '<td class="ing4">{percentage}%</td>'.format(
				percentage=ingredient['Values']['Percent'])
			start += '</tr>'
		start += '</table><br>'
		if start[-179:] == '<tr><th class="subhead">Fermentable</th><th class="subhead">Colour</th><th class="subhead">lb:oz</th><th class="subhead">Grams</th><th class="subhead2">Ratio</th></tr></table><br>':
			start = start[:-179]
		...
		for addition in self.sixth_tab.added_additions:
			try:
				if brew_data.water_chemistry_additions[addition]['Values']['Type'] == 'Hop':
					temp_hop.append(
						{'Name': addition, 'Values': brew_data.water_chemistry_additions[addition]['Values']})
				else:
		...
		temp_hop = list(sorted(
			[x for x in temp_hop if x is not None], key=lambda k: k['Values']['Time']))
		for hop in reversed(temp_hop):
			if hop['Values']['Type'] != 'Hop':
				start += '<tr>'
				start += '<td class="hop1">{name}</td>'.format(
					name=hop['Name'])
				...
				start += '<td class="hop5">{lb}:{oz}</td>'.format(
					lb=int(
						hop['Values']['lb:oz'][0]), oz=round(
						hop['Values']['lb:oz'][1], 1))
				start += '<td class="hop5">{grams}</td>'.format(grams=(round(hop['Values']['Grams'], 1)) if (
					hop['Values']['Grams'] - int(hop['Values']['Grams'])) >= 2 else round(hop['Values']['Grams']))
				...
				start += '</tr>'
		start += '</table><br>'
		if start[-236:] == '<tr><th class="subhead">Hop Variety</th><th class="subhead">Type</th><th class="subhead">Alpha</th><th class="subhead">Time</th><th class="subhead">lb:oz</th><th class="subhead">Grams</th><th class="subhead2">Ratio</th></tr></table><br>':
			start = start[:-236]
		...
		for addition in self.sixth_tab.added_additions:
			try:
				...
				if len(
					brew_data.yeast_data[addition]['Temperature'].replace(
						'°',
						'').split('-')) >= 2:
					temperature = brew_data.yeast_data[addition]['Temperature'].replace(
						'°', '').split('-')[0]
					temperature += '-' + \
						brew_data.yeast_data[addition]['Temperature'].replace('°', '').split('-')[1]
				else:
					temperature = temperature
				...
				try:
					if brew_data.water_chemistry_additions[addition]['Values']['Type'] == 'Yeast':
						start += '<tr>'
						start += '<td class="yst1">{name}</td>'.format(
							name=addition)
					...
					pass
		start += '</table>'
		if start[-245:] == '<tr><th class="subhead">Yeast</th><th class="subhead">Lab</th><th class="subhead">Origin</th><th class="subhead">Type</th><th class="subhead">Flocculation</th><th class="subhead">Attenuation</th><th class="subhead2">Temperature</th></tr></table>':
			start = start[:-245]

		start += '<p><b>Final Volume: </b>{volume} Litres</p>'.format(
			volume=self.volume.get())
		start += '<p><b>Original Gravity: </b>{og}</p>'.format(
			og=round(self.og, 1))
		start += '<p><b>Final Gravity: </b>{fg}</p>'.format(
			fg=round(self.fg, 1))
		...
		if self.seventh_tab.html_formatting.get():
			start += '''<hr><h2>Notes</h2>\n{notes}'''.format(
				notes=notes) if len(notes) >= 1 else ''
		else:
			start += '''<hr><h2>Notes</h2>\n<p>{notes}</p>'''.format(
		...
		if file != '' and file is not None and not isinstance(file, tuple):
			...
			if file.lower()[-5:] == '.berf' or file.split('/')[-1] in examples:
				...
				with open(file, 'rb') as f:
					...
					data = [line.replace(b'\xa7', b'\t').strip().decode(
						'ISO-8859-1').split('\t') for line in f]
					# print(data)
					for sublist in data:
						if sublist[0] == 'grain':
							...
							EBC = float(sublist[2])
							self.ingredients.append({'Name': sublist[1], 'Values': {
													'EBC': EBC, 'Grav': 0, 'lb:oz': (lb, oz), 'Grams': grams, 'Percent': percent}})
						elif sublist[0] == 'hop':
							...
							if not any(e[1] == 'boilvol' for e in data):
								...
								self.boil_volume_ent.insert(
									0, float(sublist[2]) * brew_data.constants['Boil Volume Scale'])
			...
			self.seventh_tab.texpert.insert('1.0', ast.literal_eval(
			...
			if file.lower()[-5:] == '.berf':
				...
				with codecs.open(file, 'w', 'ISO-8859-1', errors='ignore') as f:
					for ingredient in self.ingredients:
						...
						units = (
							ingredient['Values']['Grams'] / 1000) * brew_data.constants['Efficiency']
					...
					f.write(
						'miscel\xa7origgrav\t{origgrav}\n'.format(
							origgrav=self.og))

					notes = repr(
						self.seventh_tab.texpert.get(
							'1.0', 'end'))  # , encoding='utf8')
					# print(notes)
...
class defaults_editor(tk.Frame):
	...
	def save_all(self):
		...
		with open(resource_path('defaults.txt'), 'w') as f:
			...
			attenuation = (attenuation_type if attenuation_type !=
						   'medium' else 'med') + '-' + (attenuation_temp)
	...
	def temp_save(self):
		...
		brew_data.constants['Save On Close'] = True if self.save_on_close_var.get(
		) == 'True' else False
		brew_data.constants['Default Boil Time'] = int(
			self.default_boil_time_spinbox.get())
		brew_data.constants['Replace Defaults'] = True if self.save_on_close_var.get(
		) == 'True' else False
	...
	def new_water_chem(self):
		...
		def done():
			...
			if type_var.get() == 'Hop':
				...
				)]['Values']['Time'] = float(time_var.get())
		...
		name_var = tk.StringVar(value='New Item {num}'.format(
...
class yeast_editor(tk.Frame):
	...
	def done(self):
		...
		yeast_type = 'D' if self.yeast_type_combo.get(
		) == 'Dry' else self.yeast_type_combo.get()
