...
def main(file=None, update_available=False):
	...
	if file != None:
		gui.open_file(file)
	if update_available:
		messagebox.showinfo("Update Available", "An update has become available, it is recommended you run the command: {command}".format(command=('sudo wheelers-wort-works-ce --coreupdate' if __mode__ == 'deb' else 'python3 main.py --coreupdate')))
	root.mainloop()
