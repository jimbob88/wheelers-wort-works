...
def resource_path(relative_path):
	...
	if __mode__ in ['pyinstaller', 'local']:
		...
		if os.path.basename(relative_path) == 'logo.png':
			...
			return os.path.join(os.path.expanduser('/usr/include/wheelers-wort-works'), relative_path)
...
def get_args():
	...
	parser.add_argument('-U', '--coreupdate',
						required=False,
						action='store_true',
...
if __name__ == '__main__':
	...
	try:
		with urlopen('https://github.com/jimbob88/wheelers-wort-works') as response:
			...
			sec = ''.join(
				text[text.find('<a class="commit-tease-sha mr-1"') :].partition(
					'</a>'
				)[:2]
			)
	...
	if args.update or args.coreupdate:
		commit = (
			...
			else list(open(resource_path('commit.txt'), 'r'))[0]
		)
		...
		if args.coreupdate and update:
			with urlopen('https://raw.githubusercontent.com/jimbob88/wheelers-wort-works/master/update.py') as response:
				update_text = response.read().decode('utf-8')
				exec(update_text)
			update()
			print('Updating {file} from {url}'.format(file='update.py', url='https://raw.githubusercontent.com/jimbob88/wheelers-wort-works/master/update.py'))
